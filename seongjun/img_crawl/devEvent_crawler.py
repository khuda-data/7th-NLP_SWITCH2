...
def crawl_devEvent():
    ...
    for card in cards:
        try:
            ...
            img_elem  = card.find_element(By.CSS_SELECTOR, 'img[alt="이벤트 이미지"]')
            url = card.get_attribute("href")

            title = title_elem.text.strip()
            date = date_elem.text.strip()
            host = host_elem.text.strip()
            img_url = img_elem.get_attribute("src").strip() if img_elem else ""

            results.append({"title": title, "date": date, "host": host, "url": url, "img_url": img_url})
